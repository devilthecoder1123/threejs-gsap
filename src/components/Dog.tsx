...
const Dog: React.FC = () => {
  ...
  const model = useGLTF('/models/dog.drc.glb') as unknown as GLTF
  ...
  const [normalMap] = (useTexture(['/dog_normals.jpg']) as THREE.Texture[]).map((texture) => {
    texture.flipY = false
  ...
  const [branchMap, branchNormalMap] = (
    useTexture(['/branches_diffuse.jpeg', 'branches_normals.jpeg']) as THREE.Texture[]
  ).map((texture) => {
    texture.colorSpace = THREE.SRGBColorSpace
    return texture
  })
  ...
  const material = useRef<MatcapUniforms>({
